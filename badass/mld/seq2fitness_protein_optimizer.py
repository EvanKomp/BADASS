...
from badass.mld.protein_optimizer import BaseOptimizerParams, BaseProteinOptimizer
...
@dataclass 
class ProteinOptimizerParams(BaseOptimizerParams):
...
class ProteinOptimizer(BaseProteinOptimizer):
    ...
    def setup_model(self):
        """Initialize either ESM or Seq2Fitness model based on params."""
        if self.params.esm_only:
            self._setup_esm_model()
        else:
            self._setup_seq2fitness_model()
            
        # Move model to device and wrap in DataParallel if multiple GPUs
        if self.num_gpus > 1:
            self.model = torch.nn.DataParallel(self.model)
            print(f"Using {self.num_gpus} GPUs through DataParallel.")
    ...
    def _setup_esm_model(self):
        ...
        # Get reference logits using proper sequence conversion
        batch_tokens, rel_seqs_tensors = self._prepare_batch([''])  # This will handle proper conversion
        with torch.no_grad():
            outputs = self.model(tokens=batch_tokens)
            self.wt_logits = outputs['logits'][:, 1:-1, self.aa_indices].squeeze()
            
    def _setup_seq2fitness_model(self):
        """Initialize Seq2Fitness model and task weights."""
        self.model, self.model_params, _ = ModelCheckpoint.load_model(self.params.model_checkpoint_path)
        self.tokenizer = self.model.alphabet.get_batch_converter()
        
        if self.params.task_weights is None:
            self.task_weights = {
                task: self.model_params['task_criteria'][task]['weight']
                for task in self.model_params['task_criteria']
            }
        else:
            self.task_weights = self.params.task_weights
            
        # Get wildtype score for reference
        batch_tokens, rel_seqs_tensors = self._prepare_batch([''])
        with torch.no_grad():
            if isinstance(self.model, torch.nn.DataParallel):
                predictions = self.model.module(batch_tokens, rel_seqs_tensors)
            else:
                predictions = self.model(batch_tokens, rel_seqs_tensors)
            self.wt_score = compute_model_scores(predictions, self.task_weights).squeeze()
            
        print(f"Reference sequence has score {float(self.wt_score):.3g}")
            
    def score_batch(self, sequences: List[str]) -> List[float]:
        """Score a batch of sequences using the appropriate model."""
        batch_tokens, rel_seqs_tensors = self._prepare_batch(sequences)
        
        if self.params.esm_only:
            scores = self._compute_esm_scores(batch_tokens, rel_seqs_tensors)
        else:
            with torch.no_grad():
                if isinstance(self.model, torch.nn.DataParallel):
                    predictions = self.model.module(batch_tokens, rel_seqs_tensors)
                    ...
                    predictions = self.model(batch_tokens, rel_seqs_tensors)
                scores = compute_model_scores(predictions, self.task_weights).squeeze()
                scores = scores.detach().cpu().numpy()
                
        return scores
        
    def _compute_esm_scores(self, batch_tokens, rel_seqs_tensors) -> np.ndarray:
        """Compute ESM-based scores for sequences."""
        with torch.no_grad():
            logits = self.model(tokens=batch_tokens)['logits'][:, 1:-1, self.aa_indices]
        ...
        return (self.params.esm_score_weights['mutant'] * np.array(mutant_scores) +
                self.params.esm_score_weights['reference'] * np.array(ref_scores))
                
    def _prepare_batch(self, sequences: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
        """Prepare batch of sequences for model input."""
        # Convert to relative sequence dictionaries
        rel_seqs_dict = rel_sequences_to_dict(sequences, sep='-')
        ...
        batch_labels, batch_strs, batch_tokens = self.tokenizer(
            [(str(i), seq) for i, seq in enumerate(abs_seqs)])
            
        # Convert relative sequences to tensors
        rel_seqs_tensors = convert_rel_seqs_to_tensors(rel_seqs_list)
        rel_seqs_tensors_padded = pad_rel_seq_tensors_with_nan(rel_seqs_tensors)
        
        return batch_tokens.to(self.device), rel_seqs_tensors_padded.to(self.device)
        
    def _tokenize_sequences(self, sequences: List[str]) -> torch.Tensor:
        """Tokenize sequences for ESM model."""
        _, _, tokens = self.tokenizer([(str(i), seq) for i, seq in enumerate(sequences)])
